...
async function assertPresetSourceScanOverridesPackMeta() {
  ...
  writeFileSync(
    join(projectDir, "src", "prompt-template", "010-plain-main-user.md"),
    "new prompt",
    "utf-8"
  );
  ...
  assertEqualJson(
    ...
    "preset source scan: prompt-template is rebuilt from src"
  );
  assertEqualJson(
    builtPreset.regex,
    [regexEntry],
    "preset source scan: regex is rebuilt from src"
  );
}
...
async function assertModuleSourceScanOverridesPackMeta() {
  ...
  assertEqualJson(
    ...
    "module source scan: lorebook is rebuilt from src"
  );
  assertEqualJson(
    builtModule.regex,
    [regexEntry],
    "module source scan: regex is rebuilt from src"
  );
}
